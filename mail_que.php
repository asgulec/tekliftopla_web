...
mysqli_query($connection,"delete from mail_que where retry>3");



$r=mysqli_query($connection,"select id,email,konu,mesaj from mail_que order by id asc limit 0,50");
...
for($i=0;$i<$n;$i++) {
		...
		if(!$mail_sent)
			...
			mysqli_query($connection,"update mail_que set retry=retry+1 where id=$id");

		else

			mysqli_query($connection,"delete from mail_que where id=$id");
