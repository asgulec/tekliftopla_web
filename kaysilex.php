...
$firmaid=intval(trim(base64_decode(urldecode(isset($_GET['cex']) ? $_GET['cex'] : ''))));
//$idate=date("Ymd");
...
if($etki){
	...
	$sql = "update bilgi set lisan='Hayır' where firmaid=$firmaid LIMIT 1";
	mysqli_query($coni,$sql) or die(mysqli_error($coni));
	$sqla = "REPLACE INTO unsubsex SET undate=now(), firmaid=$firmaid";
	mysqli_query($coni,$sqla) or die(mysqli_error($coni));
	session_destroy ();
	header("location:kaysilexok.php");
	exit;
	}
}
else{
