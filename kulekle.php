...
$sure = mysqli_real_escape_string($connection,isset($_POST["sure"]) ? $_POST["sure"]: '');
$sehir = mysqli_real_escape_string($connection,isset($_POST["sehir"]) ? $_POST["sehir"]:'');
$text = mysqli_real_escape_string($connection,isset($_POST["text"]) ? $_POST["text"]:'');
$tarih= date("Y-m-d", strtotime(mysqli_real_escape_string($connection,isset($_POST["tektarih"]) ? $_POST["tektarih"]:'')));
$mesajnum = mysqli_real_escape_string($connection,isset($_POST["MesajNum"]) ? $_POST["MesajNum"]:'');
if ($tarih>date('Y-m-d',strtotime('+102 days')))
	{$tarih= date('Y-m-d',strtotime('+102 days'));
		}
...
if (isset($_SESSION["verified_kulid"]))
{
		...
		$ekle1="Update kullanim set tarih='$tarih',sure='$sure',iletisim='$iletisim',text='$text',sehirid='$sehir' where Kullanimid ='$verified_kulid'";
		...
		$hata= "Tamam";
		if ($iletisim=="Faks" and $satir['Fax']=="0") 
		{  
			$hata = "Hata";
			header("location:bilgiekle.php?bilgi=f");
			exit;
		}
		if ($iletisim=="Telefon" and $satir['Telefon']=="0") 
		{  
			$hata = "Hata";
			header("location:bilgiekle.php?bilgi=t");
			exit;
		}
		if ($iletisim=="Posta" and $satir['Adres']=="") 
		{ 
			$hata = "Hata";
			header("location:bilgiekle.php?bilgi=p");
			exit;  
		}
		if ($iletisim=="Ziyaret" and $satir['Adres']=="") 
		{ 
			$hata = "Hata";
			header("location:bilgiekle.php?bilgi=p");
			exit;  
...
else
{
		...
		//$tarih=$yil."-".$ay."-".$gun;
		$ekle1="INSERT INTO kullanim (tarih,sure,iletisim,text,firmaid,session,sehirid,date,time,mesajsay,kulip) VALUES ('$tarih','$sure','$iletisim','$text','$verified_firmaid','$verified_sifre1','$sehir','$date','$time','$mesajnum','$kulip')";
		$verified_sehirid=$sehir;
		$_SESSION['verified_sehirid']=$verified_sehirid;
		//session_register("verified_sehirid");
		$sonuc1=mysqli_query($connection,$ekle1);
